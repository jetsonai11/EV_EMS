...
def sim_drive_sub( cyc , veh , initSoc):
    ...
    s_ele0=[0]*len(cycSecs)
    s_ele1=[0]*len(cycSecs)
    s_ele2=[0]*len(cycSecs)
    s_indx=[0]*len(cycSecs)
    a_indx=[0]*len(cycSecs)
    a_ele=[0]*len(cycSecs)
...
#        EMS = 'rule'
    ...
    #         s_EM = 0.98;
            ...
            mcMechKw4ForcedFc1=np.linspace(mcminKw_temp,mcmaxKw_temp,reso)
            fsKwOutAch1 = transKwInAch[i]-mcMechKw4ForcedFc1

            #         if max(fsKwOutAch1)>fsmaxKw:
            #             mcminKw_temp = max(transKwInAch[i]-fsmaxKw,mcminKw)
            if min(fsKwOutAch1)<0:
                mcmaxKw_temp = min(transKwInAch[i]-fsminKw,mcmaxKw)
                mcMechKw4ForcedFc1=np.linspace(mcminKw_temp,mcmaxKw_temp,reso)
                fsKwOutAch1 = transKwInAch[i]-mcMechKw4ForcedFc1

            gamma = (np.sign(mcMechKw4ForcedFc1)+1)*0.5
            eff_EM = np.interp(np.abs(mcMechKw4ForcedFc1), mcKwOutArray, veh['mcFullEffArray'])
            # em_trq_tot
            #         eff_EM = np.interp(2.5, xp, fp)
            #         mcFullEffArray[m] = (mcKwOutArray

            m_equ_em_no_penalty =(s_EM*gamma/eff_EM + s_EM*(1-gamma)/eff_EM)*(mcMechKw4ForcedFc1*1000)/Qhlv;
            ...
            #         if mcMechKw4ForcedFc1[i]<0:
            # #             if mcMechKwOutAch1[i]*-1 == max(veh['mcKwInArray']):
            # #                 mcElecKwInAch1[i] = mcMechKwOutAch1[i]*veh['mcFullEffArray'][len(veh['mcFullEffArray'])-1]
            # #             else:
            #                 mcElecKwInAch1[i] = mcMechKwOutAch1[i]*veh['mcFullEffArray'][max(1,np.argmax(veh['mcKwInArray']>min(max(veh['mcKwInArray'])-0.01,mcMechKwOutAch1[i]*-1))-1)]
            #         else:
            # #             if veh['maxMotorKw'] == mcMechKwOutAch1[i]:
            # #                 mcElecKwInAch1[i] = mcMechKwOutAch1[i]/veh['mcFullEffArray'][len(veh['mcFullEffArray'])-1]
            # #             else:
            #                 mcElecKwInAch1[i] = mcMechKwOutAch1[i]/veh['mcFullEffArray'][max(1,np.argmax(veh['mcKwOutArray']>min(veh['maxMotorKw']-0.01,mcMechKwOutAch[i]))-1)]
...
#             mcMechKwOutAch[i] = max(minMcKw2HelpFc[i],mcKwIfFcIsReq[i])
    ...
    output['s_ele0']=np.array(s_ele0)
    output['s_ele1']=np.array(s_ele1)
    output['s_ele2']=np.array(s_ele2)
    output['a_ele']=np.array(a_ele)
    output['a_indx']=np.array(a_indx)
    output['s_indx']=np.array(s_indx)
    output['mpsAch']=np.array(mpsAch)
    output['curMaxMechMcKwIn']=np.array(curMaxMechMcKwIn)
